...
protected:
    inline VectorID Next_ID(uint8_t level) {
        FatalAssert(directory.size() > 1, LOG_TAG_BASIC, "Height of the tree should be at least two but is %hhu.", directory.size());
        FatalAssert((level < directory.size()) || (level == directory.size() && !directory.back().empty()), LOG_TAG_BASIC, "Input level(%hhu) should be less than height of the tree(%hhu)."
