...
public:
    ...
    bool copper_node_test() {
        ...
        ErrorAssert(update.vector_data == other_node._bucket.Get_Last_Vector().Get_Address(), LOG_TAG_TEST,
                    "Last vector address should be same as the current vector address. update.vector_data=%lu, "
                    "other_node._bucket.Get_Last_Vector().Get_Address()=%lu", update.vector_data,
