...
public:
    ...
    bool clustering() {
        ...
        copper::Buffer_Manager<uint16_t, DIM, MIN_SIZE, MAX_SIZE, MIN_SIZE, MAX_SIZE,
                               double, copper::Simple_Divide_L2> _bufmgr;
        using Node = copper::Copper_Node<uint16_t, DIM, MIN_SIZE, MAX_SIZE,
        ...
        status = status && rs.Is_OK();
            ErrorAssert(rs.Is_OK(), LOG_TAG_VECTOR_INDEX, "Failed to update cluster address for node_id: " VECTORID_LOG_FMT, VECTORID_LOG(node_id));
        uint16_t level = 2;

        copper::Copper_Node<uint16_t, DIM, MIN_SIZE, MAX_SIZE, double, copper::Simple_Divide_L2> *node =
        ...
        for (uint16_t i = 0; i < MAX_SIZE; ++i) {
            ...
            status = status && rs.Is_OK();
            ErrorAssert(rs.Is_OK(), LOG_TAG_VECTOR_INDEX, "Failed to insert vector with id: " VECTORID_LOG_FMT, VECTORID_LOG(ids[i]));
        }
        ...
        status = status && rs.Is_OK();
        ErrorAssert(rs.Is_OK(), LOG_TAG_VECTOR_INDEX, "Failed to update cluster address for root_id: " VECTORID_LOG_FMT, VECTORID_LOG(_root_id));
        rs = _bufmgr.UpdateVectorAddress(_root_id, new_root->Insert(node->Compute_Current_Centroid(), node_id));
        status = status && rs.Is_OK();
        ErrorAssert(rs.Is_OK(), LOG_TAG_VECTOR_INDEX, "Failed to update vector address for root_id: " VECTORID_LOG_FMT, VECTORID_LOG(_root_id));
        rs = node->Assign_Parent(_root_id);
        status = status && rs.Is_OK();
        ErrorAssert(rs.Is_OK(), LOG_TAG_VECTOR_INDEX, "Failed to assign parent for node_id: " VECTORID_LOG_FMT, VECTORID_LOG(node_id));
        ++level;
        ...
        status = status && rs.Is_OK();
        ErrorAssert(rs.Is_OK(), LOG_TAG_VECTOR_INDEX, "Clustering failed with error: %s", rs.Msg());
